...
class ultraChatBot():
    def __init__(self, json):
        try:
            ...
            self.ultraAPIUrl = 'https://api.ultramsg.com/instance19879/'
            self.token = 'on8a6sdqqjnhl2iq'
        except:
            self.json = json
            self.dict_messages = ''
            self.ultraAPIUrl = 'https://api.ultramsg.com/instance19879/'
    ...
    def welcome(self, chatID, noWelcome=False):
        ...
        if (noWelcome == False):
            ...
            welcome_string = """ No he comprendido lo que tratas de decirme, indicame que necesitas! """
        return self.send_message(chatID, welcome_string)

    def response_bot(self, chatID,response):
    ...
    def Processingـincomingـmessages(self):
        if self.dict_messages != []:
            ...
            if not message['fromMe']:
                chatID = message['from']
                print("bodey = ", message['body'])
                response=reply_msg(message['body'])
                if len(response)>0:
                    return self.response_bot(chatID,response)
                    """
                    if text[0].lower() == 'hi':
